        ...
        {controlsVisible && (
          <View style={styles.controlsOverlay}>
            <View style={styles.topControls}>
              ...
              <TouchableOpacity
                onPress={() => {
                  if (isFav) removeFavorite(channelId);
                  else addFavorite(channelId);
                  setIsFav(!isFav);
                }}
              >
                <Text style={styles.favoriteIcon}>{isFav ? 'HEART' : 'EMPTY'}</Text>
              </TouchableOpacity>
            </View>
...
const styles = StyleSheet.create({
  ...
  topControls: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', paddingHorizontal: 15, paddingVertical: 10 },
  backButton: { padding: 10 },
  backIcon: { color: '#fff', fontSize: 16, fontWeight: '600' },
  channelTitle: { flex: 1, color: '#fff', fontSize: 18, fontWeight: 'bold', marginHorizontal: 10, textAlign: 'center' },
  favoriteButton: { padding: 10 },
  favoriteIcon: { fontSize: 20 },
  centerControls: { justifyContent: 'center', alignItems: 'center' },
  playButton: { width: 60, height: 60, borderRadius: 30, backgroundColor: '#e74c3c', justifyContent: 'center', alignItems: 'center' },
  playIcon: { fontSize: 28, color: '#fff' },
  errorContainer: { flex: 1, justifyContent: 'center', alignItems: 'center' },
  errorOverlay: { ...StyleSheet.absoluteFillObject, backgroundColor: 'rgba(0, 0, 0, 0.7)', justifyContent: 'center', alignItems: 'center' },
  errorText: { color: '#ff4444', fontSize: 16, fontWeight: '600', textAlign: 'center' },
  backButtonError: { marginTop: 20, paddingHorizontal: 20, paddingVertical: 10, backgroundColor: '#e74c3c', borderRadius: 8 },
